...
- For `ENTER_LONG` and `ENTER_SHORT`, prefer `execution.setup_candidates` when present. Select `quantity` only from `valid_entry_quantities`, use `order_type: "MARKET"`, and include absolute numeric `stop_loss` and `take_profit_1`.
- For `MOVE_STOP`, include only `stop_loss` (absolute price) and omit entry fields. Only when `execution.move_stop_available=true`.
- For `HOLD`, `EXIT`, and `NOTHING`, omit `quantity`, `order_type`, `stop_loss`, and `take_profit_1`.
- Do not emit `MOVE_TP`, limit orders, provider IDs, credentials, additional fields, multiple objects, comments, or trailing text.
