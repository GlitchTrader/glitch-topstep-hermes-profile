...
- When flat with a single candidate, `decisive_evidence` must be the complete `INSTRUMENT_COMPARISON_V1` line ledger for that instrument (see `topstep-market-scan`), including `SELECTION_EV` with `now_ev=POSITIVE_ROBUST|POSITIVE_THIN|NEGATIVE|UNCERTAIN`. Put `prior_hypothesis` continuity in `disconfirming_evidence` when `recent_frames` is non-empty.
...
- Derived composite scores summarize underlying inputs; do not count them as independent confirmation when those inputs are evaluated directly. Reason by evidence families (structure, momentum, order flow, volatility, location, liquidity).
- For `ENTER_LONG` and `ENTER_SHORT`, state proposed risk in points, ticks, 1m/5m ATR or equivalent noise, and one-contract dollars before costs. The stop must survive ordinary movement over the intended five-to-ten-bar forecast horizon. Use `entry_band_guidance.suggested_min_width_ticks` when present as advisory minimum band width in ticks, not prices; when null, do not derive band width from guidance.
- For `ENTER_LONG` and `ENTER_SHORT`, `entry_price_min` / `entry_price_max` must contain the decision reference price (last/mid), price plausible decision-to-delivery drift once, and remain strictly between the structural `stop_loss` and primary `take_profit_1`. The band is the complete bounded zone where the setup retains positive expected value, not a one-tick quote or the raw bid/ask; never widen the range merely to defeat revalidation.
...
- Include `SELECTION_EV` in the tail with `direction=LONG|SHORT` (never `FLAT`/`NONE`/`NA`); `ENTER_*` requires `now_ev=POSITIVE_ROBUST|POSITIVE_THIN`; flat `NOTHING` forbids `now_ev=POSITIVE_ROBUST` but allows `POSITIVE_THIN` when abstaining despite thin edge; still needs that side's counterfactual entry/stop/target numbers.
