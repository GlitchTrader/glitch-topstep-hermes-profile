...
- When `market_universe.candidates` has only one entry, `decisive_evidence` must begin with `prior_hypothesis=<CONFIRMED|INVALIDATED|PARTIALLY_CONFIRMED|UNCHANGED>` when `recent_frames` is non-empty, then state material deltas since the immediately prior frame.
