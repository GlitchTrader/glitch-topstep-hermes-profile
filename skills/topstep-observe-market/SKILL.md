...
8. Depth: when `order_flow.observation.depth.available` is false, do not infer book imbalance. That belongs in `optional_issues`, not as a `state_complete` failure.
