...
- Worker cron: every minute for frame capture and cycle bookkeeping.
- Flat cognition: one LLM call every five minutes by default at minutes 0, 5, 10, … (`GLITCH_TOPSTEP_FLAT_DECISION_INTERVAL_MINUTES`); operators may change cadence explicitly. Off-cadence minutes still capture frames without invoking the model.
...
- Gateway publishes `entry_band_guidance` (advisory ticks only; null when quote geometry invalid) and omits bid/ask from `required_output_template` entry fields.
- `data_quality` may include `quote_geometry_invalid` when bid >= ask; treat BBO as timing-invalid for entry EV.
- Profile revalidates entries at delivery using executable geometry (ask for LONG, bid for SHORT) with decision-reference fallback when BBO is missing. `entry_price_min/max` is cognitive/audit-only at delivery; breach is logged via `entry_delivery_revalidation` in `events.jsonl`.
