...
CYCLE_OPERATOR_INSTRUCTION = (
    ...
    "Treat the executable market quote as timing-invalid whenever bid or ask is missing, bid >= ask, "
    "market.spread_ticks <= 0, data_quality lists quote_geometry_invalid, or quoted spread is materially "
    "inconsistent; do not derive executable entry location, friction, midpoint, or entry-band width from "
    "invalid quote geometry. When entry_band_guidance.suggested_min_width_ticks is null, ignore band-width "
    "advisory. Preserve structural cognition from usable bars and observation; separate structural data "
    "usability, flow usability (no recent trades or valid depth cannot confirm direction), and executable "
    "timing usability (invalid or stale BBO means no reliable price for current-zone entry EV — direction "
    "may remain assessable while timing is unusable). "
    ...
    "reserve UNCERTAIN for a genuinely unusable fact; otherwise classify SELECTION_EV.now_ev only as "
    "POSITIVE_ROBUST, POSITIVE_THIN, NEGATIVE, or UNCERTAIN — never legacy POSITIVE. "
    "POSITIVE_ROBUST normally supports participation subject to execution and survival constraints. "
    "POSITIVE_THIN may justify the smallest supported quantity only when the thesis is locally falsifiable, "
    "the stop survives ordinary forecast-horizon noise, execution uncertainty is bounded, the edge does not "
    "depend mainly on correlated evidence, and account survival cost is small; otherwise choose NOTHING. "
    "Treat approximately 0.03 probability above breakeven at the low end of estimated_target_first_range as "
    "a reference robustness margin, not a universal hard gate; also weigh probability-range width, evidence "
    "quality, geometry, and execution uncertainty together. "
    "Separate directional evidence (which path is more likely: structure, momentum, order flow, comparable "
    "cross-instrument context) from trade quality evidence (whether current location, invalidation, target "
    "room, noise, friction, and execution uncertainty create positive risk-adjusted EV). Strong directional "
    "conviction does not by itself justify entry from poor location or geometry. "
    "Correlated observations must not count as independent confirmation across evidence families; reason from "
    "distinct information content, not confirmation count. Evidence families are not votes with fixed weights; "
    "relevance depends on setup, regime, location, horizon, and falsification structure. "
    "First determine whether a valid trade exists independent of quantity; then direction, objective and "
    "genuine invalidation, noise and geometry, target-first probability, current-zone EV, NOW versus WAIT, "
    "cross-instrument ranking, action, and only then quantity from supported capacity. "
    "Quantity should increase only when edge robustness, geometry quality, execution quality, and survival "
    "margin jointly support it; confidence alone must not determine size; POSITIVE_THIN normally uses the "
    "smallest supported quantity when participation is justified. "
    "Cognition determines the best market thesis and hypothetical EV from usable evidence; deterministic "
    "gateway facts determine whether exposure can be transported — do not reinterpret technical execution "
    "rejection as evidence against the market thesis. "
    "Use 60m for context, "
    ...
    "put prior_hypothesis continuity in disconfirming_evidence when recent_frames is non-empty. "
    "Use packet.entry_band_guidance.suggested_min_width_ticks as advisory minimum band width in ticks when "
    "present, not as prices; when null, quote geometry is unusable for band-width guidance. Reason by evidence families (price structure, momentum, order flow, volatility, "
    ...
    "Continuity authority hierarchy: the current packet is factual ground truth; recent_glitch_ledger is "
    "the authoritative prior cognitive state; cycle_evidence_delta summarizes material change since that "
    "state; recent_frames are supplemental path evidence only. Never let prior cognition override "
    "contradictory current packet facts. "
    ...
    "use POSITIVE_ROBUST when estimated_target_first_range low exceeds breakeven_target_first by at least "
    "the reference robustness margin (~0.03 probability); breakeven_target_first=(risk_points+friction_points)/(risk_points+reward_points). "
        ...
        try:
            ...
            if "selection_ev_direction_invalid" in error_text:
                instruction += (
                    ...
                    "now_ev=NEGATIVE, UNCERTAIN, or POSITIVE_THIN only when abstaining despite thin edge."
                )
